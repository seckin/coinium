...
def create_investment(request, portfolio_id):
    ...
    new_activity(request.user.id, timestamp, "new_investment")

    webhook_url = 'https://hooks.slack.com/services/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'
    slack_data = {"text":"New investment to " + portfolio.portfolio_name + " by " + request.user.first_name + " " + request.user.last_name}
...
def newportfolio(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            timestamp = round(time.time() * 1000000)
            new_activity(request.user.id, timestamp, "new_portfolio")

            webhook_url = 'https://hooks.slack.com/services/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'
            slack_data = {"text":"New portfolio created. " + portfolio.portfolio_name + " by " + request.user.first_name + " " + request.user.last_name}
...
def profile(request, user_id):
    ...
    follow = Follow.objects.filter(owner=request_user, followed=user)
    following = True if len(follow) else False

    res = Review.objects.filter(for_user=user)
    reviews_with_profile_pics = []
    for review in res:
        if len(Document.objects.filter(user=review.owner)):
            fs = FileSystemStorage()
            profile_pic_url = fs.url(review.owner.document.document_filename)
        else:
            profile_pic_url = "https://storage.googleapis.com/indie-hackers.appspot.com/avatars/P0jgWBHyYbaC9wp1GEjGELwjsL63"
        reviews_with_profile_pics.append({"review":review, "profile_pic_url": profile_pic_url})
    ...
    review_saved = False
    if 'review_saved' in request.session and request.session['review_saved']:
...
def review(request):
    if request.method == 'POST':
        for_user_id = request.POST.get("for_user_id")
        for_user = User.objects.get(pk=for_user_id)
...
def feed_test(request):
    from .tasks import add_follower, unfollow
    add_follower(str(1), str(12))
    add_follower(str(1), str(13))
    add_follower(str(1), str(14))
    add_follower(str(1), str(15))
    add_follower(str(12), str(1))
    add_follower(str(12), str(13))
    add_follower(str(12), str(14))
    add_follower(str(12), str(15))

    # unfollow(1, 12)
    # save_activity(user_id, timestamp, activity_type)

    new_activity(str(12), round(time.time()*1000000), "new_investment")

    # version = 1
    keys = get_feed_of_user(str(14), str(1))
    # keys = get_followers(str(12))
    # keys = [str(x) for x in keys]
    return JsonResponse({"result": keys}, safe=False)

def follow(request):
    to_follow_id = request.POST.get("user_id")
    user = User.objects.get(pk=to_follow_id)
    follows = Follow.objects.filter(owner = request.user,
                                    followed = user)
    if len(follows) == 0:
        follow = Follow.objects.create(owner = request.user,
                                       followed = user)
        follow_user(user, request.user.id)
    return JsonResponse({"result": follow.id}, safe=False)

def unfollow(request):
    to_unfollow_id = request.POST.get("user_id")
    # print("to_unfollow_id", to_unfollow_id)
    user = User.objects.get(pk=to_unfollow_id)
    follows = Follow.objects.filter(owner = request.user,
                                    followed = user)
