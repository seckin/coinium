...
def new_activity(user_id, timestamp, activity_type):
    ...
    for version in VERSIONS:
        ...
        for follower in get_followers(activity['user_id']):
            # print("adding to feed of follower:", follower, "activity_type:", activity_type)
...
def write_aggr(activity, version):
    ...
    redis_connection = redis.StrictRedis(host='localhost', port=6379, db=REDIS_DB, decode_responses=True)
...
def get_feed_of_user(user_id, version):
    feed_key = "activity_profile:%s:%s" % (version, user_id,)
    redis_connection = redis.StrictRedis(host='localhost', port=6379, db=REDIS_DB, decode_responses=True)
